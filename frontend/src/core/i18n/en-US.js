...
const messages = {
  ...
  skills: {
    ...
    list: {
      ...
      // 2026-07-18 new: rename skill (globally unique)
      skillRenameTitle: 'Rename Skill',
      skillRenamePrompt: 'Skill "{name}" will be renamed to',
      skillRenameHint: 'Only the directory name is changed; the parent group stays. Allowed chars: lowercase / digits / - / _. The new name must be globally unique across all groups.',
      skillRenameOk: 'Renamed to "{name}"',
      skillRenameConflict: 'This name already exists, please use a globally unique skill name',
      skillRenameNotFound: 'Source skill no longer exists',
      skillRenameInvalid: 'Invalid name (lowercase letters / digits / - / _ only)',
...
`,
  ...
  git: {
    ...
    // 2026-07-18 增:auto-commit message settings block
    autoCommit: {
      title: 'Auto commit message',
      subtitle: 'Auto git commit on skill save; message generated by LLM or a fixed template',
      llmEnabled: {
        label: 'Auto-detect commit message via LLM',
        desc: 'When enabled, every auto-commit invokes the LLM to produce a conventional-commit message from the changed file list.',
        unavailableReason: 'LLM is currently unavailable. Configure an enabled provider in AI settings and pass the connection test.',
      },
      template: {
        label: 'Default commit template',
        desc: 'Used when LLM is disabled or the LLM call fails.',
        filename: 'By filename',
        opFiles: 'By operation + file count',
        timestamp: 'By timestamp',
      },
      testBtn: 'Test LLM',
      fileCountUnit: 'files',
      loadFail: 'Failed to load auto-commit config: {msg}',
      saved: 'Saved',
      saveFail: 'Save failed: {msg}',
      testOk: 'Test passed — {model} returned "{output}"',
