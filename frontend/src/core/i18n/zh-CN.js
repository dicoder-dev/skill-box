...
const messages = {
  ...
  skills: {
    ...
    list: {
      ...
      // 2026-07-18 增:重命名 skill(全局唯一性)
      skillRenameTitle: '重命名技能',
      skillRenamePrompt: '技能「{name}」将被重命名为',
      skillRenameHint: '只改这一段(目录名),父分组不变;合法字符:小写字母 / 数字 / - / _;新名必须全局唯一(跨所有分组)',
      skillRenameOk: '已重命名为「{name}」',
      skillRenameConflict: '该名称已存在,请换一个全局唯一的技能名称',
      skillRenameNotFound: '原技能不存在',
      skillRenameInvalid: '名称非法(仅允许小写字母 / 数字 / - / _)',
...
`,
  ...
  tools: {
    ...
    paths: {
      ...
      path: '路径',
      pathHint: '绝对路径,支持 ~/',
      pathHintGlobal: '全局路径,例如 Claude Code 用 ~/.claude/skills',
      ...
      hint: '每个 (scope × category) 格子最多配置 1 条 path;留空 = 该档位不写入。Claude Code 示例:全局 ~/.claude/skills,项目 .claude/skills。',
    },
