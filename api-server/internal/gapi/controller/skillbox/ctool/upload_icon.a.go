...
//   - 2026-07-03 修: 之前用 BindParamsHandler 走 ShouldBindWith(form),
//     但 gin 的 form binding 对 multipart.FileHeader 字段是 no-op
//     (see vendor/.../binding/form_mapping.go case multipart.FileHeader: return nil),
//     导致 File 永远是 nil、binding:"required" 一直失败。
//     改用 BindHandler + 手动 c.FormFile("file")。
package ctool
...
func UploadIcon(c *ginp.ContextPlus) {
	...
	if fileHeader.Size > maxIconSize {
		c.JSON(http.StatusRequestEntityTooLarge, gin.H{
			"error": fmt.Sprintf("icon too large: %d > %d bytes", fileHeader.Size, maxIconSize),
		})
		return
	}
	// 后缀白名单
...
func init() {
	ginp.RouterAppend(ginp.RouterItem{
		...
		Handler:        ginp.BindHandler(UploadIcon),
