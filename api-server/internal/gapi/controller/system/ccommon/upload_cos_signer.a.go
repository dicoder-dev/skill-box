...
func init() {
	ginp.RouterAppend(ginp.RouterItem{
		Path:           "/api/common/upload_cos_signer",                                   //api路径
		Handler:        ginp.BindParamsHandler(UploadCosSigner, RequestUploadCosSigner{}), //对应控制器
...
func UploadCosSigner(c *ginp.ContextPlus, requestParams *RequestUploadCosSigner) {
	userId := uint(1)
	if requestParams.FileSuffix == "" {
	...
	presignedURL, fileKey, err := stsSigner.GeneratePresignedURL(requestParams.FileSuffix)
	if err != nil {
