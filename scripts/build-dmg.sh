...
# ---- 步骤 2:创建只读 staging DMG ----
echo "=== 2. CREATE staging DMG ==="
mkdir -p "$(dirname "$OUTPUT")"
DMG_STAGING="${OUTPUT}.staging.dmg"
# 200m 对 ~80MB app bundle 留足余量;HFS+ 是 Finder 拖拽布局唯一稳的卷格式;
# UDRO 挂载时只读,Finder 视图只从 .DS_Store 读,不会被运行时修改干扰。
hdiutil create \
  -srcfolder "$APP_PATH" \
  -format UDRO \
  -volname "$VOLNAME" \
...
ATTACH_OUTPUT=$(hdiutil attach -readwrite -noverify -nobrowse "$DMG_STAGING" 2>&1)
# Apple Silicon 上 hdiutil attach 可能输出多行,提取 /Volumes/<volname>
MOUNT_POINT=$(echo "$ATTACH_OUTPUT" | awk -F'\t' '/\/Volumes\// {print $3; exit}')
[ -n "$MOUNT_POINT" ] || { echo "❌ 挂载失败: $ATTACH_OUTPUT"; exit 1; }
...
tell application "Finder"
  tell disk "$VOLNAME"
    ...
    set winOpts to container window
