...
> 源约定:仓库根 `CLAUDE.md` 第 4 / 5 条。

### 1.1 图片理解 / 分析
- **统一使用 MiniMax `understand_image` MCP**,无需跟用户确认即可调用。
- 模型自带的视觉能力 **不直接用于业务代码分析**,只作为快速兜底。
- 业务侧(读图、改 UI 调样式、看错误截图)一律走 MCP。

**反例(禁止)**:
- 用户发了一张 UI bug 截图,Claude 不调 MCP,直接根据文本描述"猜"问题在哪。
- 在记忆/memory 文件里写"通过肉眼分析图像判断……",而没用 MCP 真正调用过。

### 1.2 联网搜索
- **优先使用 MiniMax `web_search` MCP**,无需跟用户确认。
- 内部已内置常用站点(iconpark 官网、wails 文档、Vue 3 文档等),常规检索无需 WebFetch。
- WebFetch 仅用于:已知具体 URL 需要看完整页面内容时(MCP 搜索只返摘要)。

**反例**:
- 看到不熟的 API / 错误码,不去搜就开始"按印象写代码",这是被严格禁止的。
- 优先 WebFetch 抓单个 URL,但其实用 MCP 搜索几个关键词就能拿到答案。
